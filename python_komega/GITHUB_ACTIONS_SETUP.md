...
GitHub ActionsのワークフローファイルがOAuth権限の問題でプッシュできませんでした。以下の手順で手動でワークフローファイルを追加してください。

## Manual Setup Steps

### 1. GitHub Web Interface でのワークフローファイル作成

1. GitHub リポジトリの `python` ブランチに移動
2. `.github/workflows/` ディレクトリを作成（存在しない場合）
3. `python-tests.yml` ファイルを作成
4. 以下の内容をコピー&ペースト：
...
1. ワークフローファイルを保存後、GitHub Actions タブでワークフローが実行されることを確認
2. 各Pythonバージョン（3.8-3.12）でテストが実行されることを確認
3. テスト結果とアーティファクトが生成されることを確認

### 3. 追加の設定（オプション）

#### ブランチ保護ルールの設定
1. リポジトリの Settings > Branches に移動
2. `python` ブランチの保護ルールを追加
3. "Require status checks to pass before merging" を有効化
4. "Python Library Tests" ワークフローを必須チェックとして追加

#### バッジの追加
README.md に以下のバッジを追加：

```markdown
[![Python Tests](https://github.com/k-yoshimi/Komega/workflows/Python%20Library%20Tests/badge.svg)](https://github.com/k-yoshimi/Komega/actions/workflows/python-tests.yml)
...
### OAuth権限エラー
- GitHub CLI を使用してワークフローファイルを追加する方法：
  ```bash
  gh workflow create .github/workflows/python-tests.yml
  ```
...
- [ ] ワークフローファイルが正常に作成された
- [ ] GitHub Actions でテストが実行される
- [ ] 全Pythonバージョンでテストが成功する
- [ ] アーティファクトが正常に生成される
- [ ] ドキュメントが正常にビルドされる
- [ ] コード品質チェックが実行される
