...
class KomegaCOCG:
    ...
    def update(self, v12: np.ndarray, v2: np.ndarray, x: np.ndarray, 
        ...
        self.vals_c.rho = zdotuMPI(self.params.ndim, v2, v2)
        ...
        alpha_denom = zdotuMPI(self.params.ndim, v2, v12) - self.vals_c.beta * self.vals_c.rho / self.vals_c.alpha
...
def komega_COCG_init(ndim: int, nl: int, nz: int, z: np.ndarray, 
                     itermax: int, threshold: float, comm: Optional[int] = None) -> np.ndarray:
    """Initialize the COCG solver."""
    return _global_cocg.init(ndim, nl, nz, z, itermax, threshold, comm)


def komega_COCG_update(v12: np.ndarray, v2: np.ndarray, x: np.ndarray, 
                       r_l: np.ndarray, status: List[int]) -> None:
    """Update the COCG iteration."""
