...
class KomegaCGR:
    ...
    def shifted_equation(self, r_l: np.ndarray, x: np.ndarray) -> None:
        ...
        for iz in range(self.params.nz):
            ...
            # Update π values
            pi_new = (1.0 + self.vals_r.alpha * (self.vals_r.z[iz] - self.vals_r.z_seed)) * self.vals_r.pi[iz] - \
                     self.vals_r.alpha * self.vals_r.beta / self.vals_r.alpha_old * \
                     (self.vals_r.pi_old[iz] - self.vals_r.pi[iz])
            
            # Update search direction
            self.vecs_r.p[:, iz] = r_l / self.vals_r.pi[iz] + \
                                 (self.vals_r.pi_old[iz] / self.vals_r.pi[iz])**2 * \
                                 self.vals_r.beta * self.vecs_r.p[:, iz]
            
            # Update solution
            daxpy(self.vals_r.pi[iz] / pi_new * self.vals_r.alpha, 
                  self.vecs_r.p[:, iz], x[:, iz])
    ...
    def seed_switch(self, v2: np.ndarray, status: List[int]) -> None:
        ...
        unconverged_indices = [i for i, conv in enumerate(self.params.lz_conv) if not conv]
        ...
        if status[2] != self.params.iz_seed:
            ...
            # Update α and ρ
            self.vals_r.alpha *= self.vals_r.pi_old[self.params.iz_seed] / self.vals_r.pi[self.params.iz_seed]
    ...
    def update(self, v12: np.ndarray, v2: np.ndarray, x: np.ndarray, 
               r_l: np.ndarray, status: List[int]) -> None:
        """
        ...
        alpha_denom = ddotMPI(self.params.ndim, v2, v12) - self.vals_r.beta * self.vals_r.rho / self.vals_r.alpha
        ...
        v12[:] = (1.0 + self.vals_r.alpha * self.vals_r.beta / self.vals_r.alpha_old) * v2 - \
                 self.vals_r.alpha * v12 - \
                 self.vals_r.alpha * self.vals_r.beta / self.vals_r.alpha_old * self.vecs_r.v3
...
def komega_CG_R_init(ndim: int, nl: int, nz: int, z: np.ndarray, 
                     itermax: int, threshold: float, comm: Optional[int] = None) -> np.ndarray:
    """Initialize the CG solver."""
    return _global_cg_r.init(ndim, nl, nz, z, itermax, threshold, comm)


def komega_CG_R_update(v12: np.ndarray, v2: np.ndarray, x: np.ndarray, 
                       r_l: np.ndarray, status: List[int]) -> None:
    """Update the CG iteration."""
