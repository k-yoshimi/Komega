...
class DetailedTestSystem:
    ...
    def setup_test_parameters(self):
        ...
        if self.test_case == "solve_cc":
            ...
            self.threshold = 1e-3
            self.rnd_seed = 100
            self.z_frequencies = [-2.0 + 1.0j, -1.0 + 1.0j, 0.0 + 1.0j, 1.0 + 1.0j, 2.0 + 1.0j]
    ...
    def run_solver_test(self) -> Dict[str, Any]:
        ...
        for iter in range(1, self.itermax + 1):
            ...
            iteration_results.append({
                'iteration': iter,
                'status': status.copy(),
                'residual_norm': np.abs(v12[0]) if self.data_type == complex else v12[0],
...
def run_all_tests():
    ...
    try:
        ...
        for test_case, results in all_results.items():
            ...
            print(f"{test_case:12s}: Status={status[0]:3d} {status[1]:3d} {status[2]:3d}, "
...
def compare_with_fortran_results(python_results: Dict[str, Any]) -> None:
    ...
    for test_case, results in python_results.items():
        ...
        if test_case in expected_results:
            ...
            if (results['convergence_status'] == expected['convergence_status'] and
                len(results['iteration_results']) == expected['final_iteration']):
