...
from .komega_math import get_global_math, zdotcMPI, zaxpy, zcopy, zscal, dscal
...
class KomegaCGC:
    ...
    def shifted_equation(self, r_l: np.ndarray, x: np.ndarray) -> None:
        ...
        for iz in range(self.params.nz):
            ...
            self.vecs_c.p[:, iz] = r_l / self.vals_r.pi[iz] + \
                                 (self.vals_r.pi_old[iz] / self.vals_r.pi[iz])**2 * \
                                 self.vals_r.beta * self.vecs_c.p[:, iz]
            
            # Update solution
            zaxpy(self.vals_r.pi[iz] / pi_new * self.vals_r.alpha, 
    ...
    def update(self, v12: np.ndarray, v2: np.ndarray, x: np.ndarray, 
        ...
        v12[:] = (1.0 + self.vals_r.alpha * self.vals_r.beta / self.vals_r.alpha_old) * v2 - \
                 ...
                 self.vals_r.alpha * self.vals_r.beta / self.vals_r.alpha_old * self.vecs_c.v3
...
def komega_CG_C_init(ndim: int, nl: int, nz: int, z: np.ndarray, 
                     itermax: int, threshold: float, comm: Optional[int] = None) -> np.ndarray:
    """Initialize the CG solver."""
    return _global_cg_c.init(ndim, nl, nz, z, itermax, threshold, comm)


def komega_CG_C_update(v12: np.ndarray, v2: np.ndarray, x: np.ndarray, 
                       r_l: np.ndarray, status: List[int]) -> None:
    """Update the CG iteration."""
