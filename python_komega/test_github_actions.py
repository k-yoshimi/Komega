...
class GitHubActionsTestSuite:
    ...
    def test_imports(self):
        ...
        try:
            ...
            self.add_result("Import parameters", True, f"Global params: {type(params).__name__}")
            
            # Test math module
            math_ops = get_global_math()
            self.add_result("Import math", True, f"Math operations: {type(math_ops).__name__}")
            
            # Test value storage modules
            vals_r = get_global_vals_r()
            vals_c = get_global_vals_c()
            self.add_result("Import value storage", True, f"Real: {type(vals_r).__name__}, Complex: {type(vals_c).__name__}")
            
            # Test vector storage modules
            vecs_r = get_global_vecs_r()
            vecs_c = get_global_vecs_c()
            self.add_result("Import vector storage", True, f"Real: {type(vecs_r).__name__}, Complex: {type(vecs_c).__name__}")
        ...
        try:
            ...
            for solver_type in solvers:
                try:
                    ...
                    self.add_result(f"Create {solver_type} solver", True, f"Type: {type(solver).__name__}")
                except Exception as e:
                    self.add_result(f"Create {solver_type} solver", False, f"Error: {e}")
    ...
    def test_mathematical_operations(self):
        ...
        try:
            ...
            self.add_result("Complex conjugate dot", True, f"Result: {dotc_complex:.6f}")
    ...
    def test_vector_storage(self):
        ...
        try:
            ...
            self.add_result("Real vector storage", True, f"v3: {v3_shape}, p: {p_shape}")
            ...
            self.add_result("Complex vector storage", True, f"v3: {v3_shape}, p: {p_shape}")
        ...
        try:
            ...
            for solver_type in solvers:
                try:
                    ...
                    self.add_result(f"Solver info {solver_type}", True, 
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            try:
                ...
                self.add_result("Invalid solver type", False, "Should have raised ValueError")
            except ValueError:
                self.add_result("Invalid solver type", True, "Correctly raised ValueError")
            
            # Test solver operations without initialization
            try:
                solver = create_solver('bicg')
                solver.update([], [], [], [], [], [], [])
                self.add_result("Uninitialized solver", False, "Should have raised RuntimeError")
            except RuntimeError:
                self.add_result("Uninitialized solver", True, "Correctly raised RuntimeError")
    ...
    def test_memory_management(self):
        ...
        try:
            ...
            self.add_result("Multiple solver instances", True, f"Created: {len(solvers)}")
...
def main():
    ...
    parser.add_argument("--test-type", choices=["basic", "solvers", "math", "storage", "compatibility", "all"],
