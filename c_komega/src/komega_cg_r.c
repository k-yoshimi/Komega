/**
 * @file komega_cg_r.c
 * @brief Real CG solver implementation for Komega C library
 * 
 ...
 * 
 * Allocates memory for a new real CG solver structure and initializes it
 * with default values.
 * 
...
struct komega_cg_r* komega_cg_r_create(void) {
    struct komega_cg_r* solver = malloc(sizeof(struct komega_cg_r));
    if (solver == NULL) {
        return NULL;
    }
 ...
 * 
 * Frees the memory allocated for the real CG solver structure and all
 * associated vectors.
 * 
...
komega_status_t komega_cg_r_init(struct komega_cg_r* solver,
    ...
    solver->r = malloc(sizeof(double) * vector_size);
    solver->p = malloc(sizeof(double) * vector_size);
    solver->v = malloc(sizeof(double) * vector_size);
    ...
    solver->r_l_save = malloc(sizeof(double) * nl * nz * (itermax + 1));
...
komega_status_t komega_cg_r_restart(struct komega_cg_r* solver,
    ...
    if (r_l_save != NULL) {
        memcpy(solver->r_l_save, r_l_save, 
               sizeof(double) * nl * nz * (itermax + 1));
    }
    
    /* Restore vectors */
    if (v2 != NULL) {
        memcpy(solver->r, v2, sizeof(double) * ndim * nz);
    }
    if (v12 != NULL) {
        memcpy(solver->p, v12, sizeof(double) * ndim * nz);
    }
 ...
 * 
 * Performs one iteration of the real CG solver algorithm.
...
komega_status_t komega_cg_r_update(struct komega_cg_r* solver,
    ...
    if (v2 != NULL) {
        memcpy(solver->r, v2, sizeof(double) * solver->ndim * solver->nz);
    }
    
    /* Update search direction vector */
    if (v12 != NULL) {
        memcpy(solver->p, v12, sizeof(double) * solver->ndim * solver->nz);
    }
...
komega_status_t komega_cg_r_getcoef(struct komega_cg_r* solver,
    ...
    if (r_l_save != NULL) {
        memcpy(r_l_save, solver->r_l_save, 
               sizeof(double) * solver->nl * solver->nz * (solver->itermax + 1));
    }
...
komega_status_t komega_cg_r_getvec(struct komega_cg_r* solver, double* r_old) {
    ...
    if (r_old != NULL) {
        memcpy(r_old, solver->r, sizeof(double) * solver->ndim * solver->nz);
    }
    ...
    if (res != NULL) {
        ...
        for (int iz = 0; iz < solver->nz; iz++) {
            ...
            for (int i = 0; i < solver->ndim; i++) {
                ...
                double abs_val = fabs(solver->r[idx]);
...
komega_status_t komega_cg_r_finalize(struct komega_cg_r* solver) {
     ...
     * for restart functionality. The user should call komega_cg_r_destroy()
