/**
 * @file test_c_solve_cr.c
 * @brief C test for Complex-Real solver (equivalent to solve_cr.F90)
 * 
 * This test program reproduces the functionality of the Fortran solve_cr.F90
...
static double complex *ham;         /* Hamiltonian matrix (complex) */
static double complex *rhs;        /* Right-hand side vector (complex) */
static double complex *v12, *v2;    /* Working vectors (complex) */
static double complex *r_l;         /* Projected residual vector (complex) */
static double complex *x;           /* Solution vector (complex) */
static double *alpha_save, *beta_save; /* Saved coefficients (real) */
static double complex *r_l_save;   /* Saved residual vectors (complex) */
...
void initialize_test_parameters(void) {
    ...
    printf("=== C Test: Complex-Real Solver ===\n");
    printf("Parameters:\n");
    printf("  ndim = %d\n", ndim);
    printf("  nz = %d\n", nz);
    printf("  nl = %d\n", nl);
...
void allocate_test_arrays(void) {
    ...
    /* Allocate saved residual vectors (complex) */
...
void generate_test_data(void) {
    ...
    /* Generate Hamiltonian matrix (complex, Hermitian) */
    ...
    /* Generate right-hand side vector (complex) */
    for (int i = 0; i < ndim; i++) {
    ...
    /* Initialize working vectors (complex) */
    for (int i = 0; i < ndim; i++) {
        v12[i] = 0.0 + 0.0 * I;
        v2[i] = 0.0 + 0.0 * I;
    }
    
    /* Initialize projected residual vector (complex) */
    for (int i = 0; i < nl; i++) {
        r_l[i] = 0.0 + 0.0 * I;
    }
    
    /* Initialize solution vector (complex) */
    ...
    /* Initialize saved residual vectors (complex) */
...
void run_cg_c_test(void) {
    ...
    /* Initialize solver */
    komega_status_t status = komega_cg_c_init(solver, ndim, nl, nz, x, z, 
                                            itermax, threshold, 0);
 ...
 */
int main(void) {
    printf("C Komega Library - Complex-Real Solver Test\n");
    printf("============================================\n\n");
